...
class QueryMemberGradeHistoryControllerTest {
    ...
    void getMemberGrades_fail_invalidPeriodCondition() throws Exception {
        ...
        Mockito.when(queryMemberGradeHistoryService.findByLoginId(eq(loginId), any()))
        ...
        verify(queryMemberGradeHistoryService, times(1)).findByLoginId(
                anyString(),
                captor.capture()
        );
    ...
    void getMemberGrades_fail_memberNotFound() throws Exception {
        ...
        List<MemberGradeHistoryQueryResponseDto> response = getMemberGradeHistoryQueryResponseData(
                loginId);
        Mockito.when(queryMemberGradeHistoryService.findByLoginId(eq(loginId), any()))
    ...
    void getMemberGrades() throws Exception {
        ...
        result.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$[0].updateDate", equalTo(updateDate.toString())))
                .andExpect(jsonPath("$[0].previousPaidAmount", equalTo((int) previousPaidAmount)))
                .andExpect(jsonPath("$[0].memberGrade", equalTo(memberGrade.name())))
        ...
        result.andDo(document(
                ...
                responseFields(
                        fieldWithPath("[].id").type(JsonFieldType.NUMBER).description("회원등급 변경 내역 Pk"),
                        fieldWithPath("[].updateDate").type(JsonFieldType.STRING)
                                .description("회원등급 변경 내역일"),
                        fieldWithPath("[].previousPaidAmount").type(JsonFieldType.NUMBER)
    ...
    private List<MemberGradeHistoryQueryResponseDto> getMemberGradeHistoryQueryResponseData(String loginId) {
