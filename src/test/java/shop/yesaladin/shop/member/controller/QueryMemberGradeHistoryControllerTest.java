...
class QueryMemberGradeHistoryControllerTest {
    ...
    void getMemberGrades_fail_invalidPeriodCondition() throws Exception {
        ...
        Mockito.when(queryMemberGradeHistoryService.getByLoginId(any(), any(), any()))
        ...
        result.andDo(document(
                ...
                requestParameters(
                        ...
                        parameterWithName("size").description("데이터 개수")
                                .optional()
                                .attributes(defaultValue(0)),
                        parameterWithName("startDate").description("회원등급 내역 조회 시작일"),
                        parameterWithName("endDate").description("회원등급 내역 조회 끝일")
                ),
                responseFields(
    ...
    void getMemberGrades_fail_memberNotFound() throws Exception {
        ...
        assertThat(ReflectionUtils.tryToReadFieldValue(
                ...
                "startDate",
                captor.getValue()
        ).get()).isEqualTo(startDate);
        assertThat(ReflectionUtils.tryToReadFieldValue(
                PeriodQueryRequestDto.class,
                "endDate",
                captor.getValue()
        ).get()).isEqualTo(endDate);
    ...
    void getMemberGrades() throws Exception {
        ...
        result.andExpect(status().isOk())
                ...
                .andExpect(jsonPath(
                        "$.data.dataList.[0].updateDate",
                        equalTo(updateDate.toString())
                ))
                .andExpect(jsonPath(
